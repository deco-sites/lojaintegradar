...
function ResultRow({ label, userValueId, lojaIntegradaValue, lojaIntegradaValueId }: {
    label: string;
    userValueId?: string;
...
function TcoCalculatorPage4({ page1, rootId, page4 }: {
    ...
    const { contentTitle, contentTitleIcon, contentCaption, asideTopIcon, mobileTopBanner, title } = page1;
        ...
        `}} /> */}
                        <form class="mt-5 xl:max-w-64 flex flex-col gap-5 z-10 items-center w-full">
                            <label>
                                ...
                                <input id={rootId + 'whatsappName'} class={inputClass} type="text" required placeholder={whatsappNameInput.placeholder} disabled={false}>
                                </input>
                            </label>
                            <label>
                                <div class={inputCaptionClass}>    
                                ...
                                <input id={rootId + 'whatsappNumber'} class={inputClass} type="tel" placeholder={whatsappNumberInput.placeholder} required>
                                </input>
                            </label>
                            <div>
                                <a id={whasappCta.id} href={whasappCta?.href ?? "#"} hx-on:click={useScript(sendWhatsapp, rootId)} target={whasappCta?.href.includes("http") ? "_blank" : "_self"} class={`btn btn-primary ${whasappCta.outline ? "btn-outline" : ""} font-bold px-5 hover:scale-110 text-lg h-auto w-auto`}>
                    ...
                    <div class="flex gap-4 justify-center xl:justify-start">
                        <div class="flex flex-col gap-y-5">
                            <div id={rootId + "negativeScreenExtraBenefit1"} class="relative max-w-[226px] min-h-[125px] px-2 py-5 hidden">
                                ...
                                {negativeScreenExtraBenefit1.image && <Image src={negativeScreenExtraBenefit1.image.src} alt={negativeScreenExtraBenefit1.image.alt || "background"} width={226} class="h-full w-full object-fill absolute top-0 left-0"/>}
                            </div>
                            <div class="relative max-w-[226px] min-h-[238px] px-2 py-5">
                                ...
                                {benefit1.image && <Image src={benefit1.image.src} alt={benefit1.image.alt || "background"} width={226} class="h-full w-full object-fill absolute top-0 left-0"/>}
                            </div>
                        </div>
                        <div class="flex flex-col justify-end gap-y-5">
                            <div class="relative max-w-[140px] min-h-[163px] px-2 py-5">
                                ...
                                {benefit2.image && <Image src={benefit2.image.src} alt={benefit2.image.alt || "background"} width={140} class="h-full w-full object-fill absolute top-0 left-0"/>}
                            </div>
                            <div id={rootId + "negativeScreenExtraBenefit2"} class="relative max-w-[140px] min-h-[191px] h-full px-2 py-5 hidden">
                                ...
                                {negativeScreenExtraBenefit2.image && <Image src={negativeScreenExtraBenefit2.image.src} alt={negativeScreenExtraBenefit2.image.alt || "background"} width={140} class="h-full w-full object-fill absolute top-0 left-0"/>}
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="py-14 px-3.5 xl:px-28 relative w-full">
                ...
                <div class="flex gap-2.5">
                    ...
                    <div class="flex justify-center bg-primary py-[18px] sm:px-3 max-w-[193px] w-[32%] 2xl:w-full rounded-bl-[20px] rounded-br-[20px]">
                        <a id={result.migrateCta.id} href={result.migrateCta?.href ?? "#"} target={result.migrateCta?.href.includes("http") ? "_blank" : "_self"} class={`btn ${result.migrateCta.outline ? "bg-primary border border-secondary-content text-secondary-content" : "bg-primary-content"}  font-bold hover:scale-110 text-lg h-auto w-full px-3.5`}>
                            {result.migrateCta.icon && <Image width={20} height={20} src={result.migrateCta.icon.src} class="h-5 w-5"/>}
