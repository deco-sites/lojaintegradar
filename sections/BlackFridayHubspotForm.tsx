...
const openAndCloseDropdown = () => {
    const modal = document.getElementById("blackFridayHubspotForm") as HTMLElement;
        ...
        if (currentPage == 1) {
            ...
            if (validated) {
                ...
                const objectToSend = {
                    firstname: data.name,
                    email: data.email,
                    qual_seu_site__url_: data.url_loja,
                    e_lojista_da_loja_integrada_: data.possui_loja
                };
...
export function CustomRadio({ name, value, type }: {
    value?: string;
    name: string;
    type?: string;
}) {
    return <>
    <input type={type || "radio"} name={name} value={value} class="hidden peer"/>
...
/** @title {{text}} */
interface Button extends submitButton {
  href?: string;
}
...
export default function BlackFridayHubspotForm({title, titleTextProps, text, textProps, submitButton, successPage = {cta: []}}:Props) {
    ...
    <form class="max-w-[582px] w-full h-fit p-5 lg:px-20 flex flex-col justify-center bg-white max-h-screen overflow-auto rounded-[20px] animate-pop-up relative" style={{ animationDuration: "0.3s" }}>
      ...
      <div class="page1">
        ...
        <div class="mb-5">
          <div class="flex gap-5" hx-on:click={useScript(onKeyUp)}>
            <label class="flex gap-2">
              <CustomRadio name="possui_loja" value="Sim"/>
              Sim
            </label>
            <label class="flex gap-2">
              <CustomRadio name="possui_loja" value="Não"/>
              Não
            </label>
          </div>
        ...
        <div class="flex w-full gap-5 justify-center mt-5">
          <button 
            type="submit" 
            style={{letterSpacing: '-0.3px', background: submitButton?.backgroundColor, color: submitButton?.textColor, borderColor: submitButton?.borderColor}} 
            class="py-[14px] px-5 text-center text-sm bg-[#28C3C3] rounded-md font-semibold text-white hover:scale-105 transition-all nextButton">
            {submitButton?.text || 'Entrar na lista de espera'}
          </button>
        </div>
      ...
      <div class="page2 flex flex-col items-center hidden">
        ...
        <div class=" text-[#5F6E82] text-base font-normal mb-5 text-center leading-tight" style={{...successPage?.text2Props}} dangerouslySetInnerHTML={{__html: successPage?.text2 || ""}}/>
        ...
        <div class="w-full">
          ...
          <a 
            style={{letterSpacing: '-0.3px', background: successPage?.closeButton?.backgroundColor, color: successPage?.closeButton?.textColor, borderColor: successPage?.closeButton?.borderColor}} 
            class="block py-[14px] w-full px-5 text-center text-sm bg-transparent cursor-pointer rounded-md font-semibold text-[#00363A] border border-[#00363A] hover:!scale-105 transition-all listaEsperaSucessButton">
              {successPage?.closeButton?.text || 'Voltar a navegação'}
          </a>
        </div>
      </div>
