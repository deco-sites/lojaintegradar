...
function SliderItem({ slide, id }: {
    ...
    return (<div id={id} class={`relative w-full h-[400px] sm:h-[484px] rounded-[30px] overflow-y-auto p-6 md:p-10 ${textColor == 'Dark' ? 'text-primary' : 'text-primary-content'}`}>
...
function Carousel(props: Props) {
    ...
    return (<div id="MainCarousel" class="relative}">
            {/* <input type="text" value="0" /> */}
            <div id={id} class="min-h-min flex flex-col items-center w-full pt-7 lg:pt-14">
                ...
                <Slider class="carousel carousel-center w-full col-span-full row-span-full gap-[30px] pl-[30px] pr-[22px] py-9 md:px-9 max-w-[1332px]" rootId={id} interval={interval && interval * 1e3} infinite id="carouselSlider">
                    {slides?.map((slide, index) => (<Slider.Item index={index} class="carousel-item w-full sm:w-[456px]">
                            <SliderItem slide={slide} id={`${id}::${index}`}/>
                        </Slider.Item>))}
                    <Slider.Item index={slides?.length || 0} class="carousel-item w-full md:w-[456px] hidden sm:block">
                        <div></div>
                    </Slider.Item>
                </Slider>
                ...
                {cta && <div id="carouselButtons" class="flex justify-center gap-7 mt-4">
                    {cta.map((item) => (<a key={item?.id} id={item?.id} href={item?.href ?? "#"} target={item?.href.includes("http") ? "_blank" : "_self"} class={`btn btn-primary ${item.outline ? "btn-outline" : ""} font-bold px-7 hover:scale-110 text-lg`}>
                            {item?.text}
                        </a>))}
                </div>}
            </div>
