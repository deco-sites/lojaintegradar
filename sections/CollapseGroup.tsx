...
export default function CollapseGroup({ hideSection, caption, title, collapses, captionColor, titleColor, collapsesBorderColor, collapsesTextColor}: Props) {
    ...
    return <div class="mt-32 lg:px-24 text-primary leading-[120%] hidden md:block" style={{color: collapsesTextColor}}>
        ...
        {collapses.map((collapse) => (<div className="collapse border border-base-200 rounded-[10px] mt-[30px]" style={{borderColor: collapsesBorderColor}}>
            <input type="checkbox" hx-on:change={useScript(onChange)}/>
            <div className="collapse-title text-xl font-medium flex px-7">
                {collapse.headerItems.map((headerItem) => (<div class={`text-center ${headerItem.textPosition == "left" && '!text-left'} ${headerItem.textPosition == "right" && '!text-right'} px-1 break-words min-w-0 leading-[120%] ${headerItem.bold ? 'text-lg lg:text-xl font-semibold' : 'text-base lg:text-lg font-normal secondaryHeaderItem hidden'}`} style={{ width: `${100 / collapse.headerItems.length}%` }}>
                    {headerItem.text}
                </div>))}

                <div class="absolute top-6 right-4 transition-all duration-300 collapse-arrow">
                <svg width="22" height="12" viewBox="0 0 22 12" class="text-primary fill-current" style={{color: collapsesTextColor}} xmlns="http://www.w3.org/2000/svg">
            ...
            <div className="collapse-content px-0">
                {collapse.rows.map((row) => (<div class="flex px-7 border-t border-t-base-200" style={{borderColor: collapsesBorderColor}}>
                    {row.items.map((rowItem) => (<div class="px-1 py-7 break-words flex flex-col items-center justify-center" style={{ width: `${100 / row.items.length}%` }}>
                        ...
                        {rowItem.use == 'check icon' && collapse.checkIcon && collapse.checkIcon.src && <img height={18} width={18} src={collapse.checkIcon.src} alt={collapse.checkIcon.alt || "check icon"} loading={"lazy"}/>}
                        {rowItem.use == 'cross icon' && collapse.crossIcon && collapse.crossIcon.src && <img height={18} width={18} src={collapse.crossIcon.src} alt={collapse.crossIcon.alt || "cross icon"} loading={"lazy"}/>}
