...
export default function BackToTopButton({ text, textColor, backgroundColor, appearAt, size, position }: Props) {
    return <a
        href="#"
        id="backToTopButton"
        class="w-[84px] min-h-[84px] fixed bottom-5 lg:bottom-14 right-5 lg:right-14 z-40 bg-primary rounded-full flex flex-col items-center justify-center p-4 gap-y-1 hover:scale-110 transition-transform hidden"
        style={{ background: backgroundColor, width: size, minHeight: size, bottom: position?.distanceFromScreenBottom, right: position?.distanceFromScreenRightEnd }}
    >
