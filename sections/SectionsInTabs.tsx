...
export default function SectionInTabs({ hideSection, id, sectionTabs = [] }: Props) {
    ...
    return <div id={id} class="pt-[71px] lg:pt-[120px]">
        <div id={rootId}>
            ...
            return <li>
                        <a hx-on:click={useScript(onClick, rootId, index, sectionTabs)} class={`sectionTab block cursor-pointer py-2.5 px-2 lg:px-3.5 rounded-full font-normal text-lg lg:text-[32px]`} style={{ background: titleBackgroundColor, fontFamily: tab.titleFont, color: textColor }}>
                            {tab.title}
                        </a>
                    </li>;
        })}
            ...
            <div>
                {sectionTabs.map((sectionTab, index) => (<div class={`${index && 'hidden'} sectionContainer`}>
