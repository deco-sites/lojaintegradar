    ...
    return (<div id="detailedCarousel" class="relative mt-16">
            ...
            <div id={id} class="min-h-min flex items-center flex-col lg:container md:max-w-[1220px] lg:mx-auto pt-16 pb-24 lg:pt-24">
                ...
                {cta && <AnimateOnShow animation="animate-fade-up" divClass="flex flex-wrap justify-center gap-7 mt-4">
                    {cta.map((item) => (<a key={item?.id} id={item?.id} href={item?.href ?? "#"} target={item?.href.includes("http") ? "_blank" : "_self"} class={`btn btn-primary ${item.outline ? "btn-outline" : ""} font-bold px-7 hover:scale-110 text-lg`} hx-on:click={item.href == '/talkToSpecialist' && useScript(openPopUpForm, id)}>
                            {item?.text}
                        </a>))}
                </AnimateOnShow>}
