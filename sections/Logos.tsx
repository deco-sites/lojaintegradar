...
export default function Logos({ title, caption, logos = IMG_PLACEHODLER, titleColor, captionColor }: Props) {
  logos = [...logos, ...logos];
    ...
    <div class="flex flex-col gap-10">
      <div id="logosTitleAndCaption">
        <AnimateOnShow animation="animate-fade-up">
          <p class="text-lg md:text-[32px] text-primary font-semibold " style={{ color: titleColor }}>{title}</p>
        </AnimateOnShow>
        <AnimateOnShow animation="animate-fade-up" delay={200}>
          <p class="text-base md:text-2xl font-light mt-4" style={{ color: captionColor }}>{caption}</p>
        </AnimateOnShow>
      </div>
