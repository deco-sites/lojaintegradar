    ...
    <>
      {media?.use == "image" && media.image?.src && (
        <Image
          ...
          style={{
            aspectRatio: media.image.width || 534 + "/" + media.image.height ||
              534,
          }}
        />
      )}
      {media?.use == "video" && media.video?.src && (
        <video
          ...
          class="object-cover"
          style={{
            width: media.video.width + "px" || "1280px",
            height: media.video.height + "px" || "720px",
          }}
        >
...
export default function HeroV3(
  ...
  return (
    <div
      class="px-5 lg:px-0 relative py-12"
      style={{
        background: sectionBackground?.backgroundColor,
        paddintTop: container?.marginTop,
        paddingBottom: container?.marginBottom,
      }}
    >
      <div
        class={`max-w-[1288px] relative z-10 mx-auto rounded-[20px] overflow-hidden p-[30px] lg:p-12 flex gap-5 lg:gap-y-9 lg:flex-nowrap items-center justify-center ${
      ...
      >
        <AnimateOnShow
          ...
          divClass={`${
            (media?.use && media.placement != "bellow") && "max-w-[448px]"
        ...
        >
          ...
          {text && (
            <div
              ...
              class="text-sm lg:text-lg w-full pb-1"
              style={{ ...textProps }}
            />
          )}
          ...
          {cta.length > 0 && (
            <div
              class={`flex flex-wrap gap-4 mt-auto ${
            ...
            >
              {cta.map((cta) => (
                <CTA
                  customEvent={"HeroV3"}
                  customEventCategory={"loja-integrada:institucional"}
                  customEventLabel={"criar-loja-gratis"}
                  {...cta}
                />
              ))}
            </div>
