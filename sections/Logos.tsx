...
const IMG_PLACEHODLDER = Array(30).fill(0).map(() => ({
...
export default function Logos({ 
  ...
  // Duplica os logos para o efeito de slider infinito
  logos = [...logos, ...logos];
  
  // Calcula a altura máxima baseada nos logos
  const maxLogoHeight = Math.max(...logos.map(logo => logo.height || 100));
        ...
        return (
          <div class="w-28 md:w-[156px] md:h-[29px] flex-shrink-0" key={index}>
            <Image 
              ...
              loading="lazy"
              fetchPriority="low"
              class={`${blackAndWhiteLogos && 'grayscale opacity-60'} h-full w-full object-contain`} 
            />
          </div>
        );
      })}
  ...
  return (
    <div 
      class="lg:container md:max-w-[1260px] lg:mx-auto px-4 md:px-0 overflow-hidden" 
      style={{ 
      ...
      <div class="flex flex-col">
        {title && (
          <AnimateOnShow animation="animate-fade-up">
            <div 
              class="text-lg md:text-[32px] text-primary font-normal leading-normal" 
        ...
        {caption && (
          <AnimateOnShow animation="animate-fade-up" delay={200}>
            <div 
              class="text-base md:text-2xl font-light" 
        ...
        <AnimateOnShow 
          divClass="relative w-full overflow-hidden mt-10" 
          animation="animate-fade-up"
          style={{ minHeight: `${maxLogoHeight}px` }}
        >
