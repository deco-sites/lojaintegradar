...
export default function Header2({ logo = {
  ...
  <header>
    ...
    <div class="fixed top-0 left-0 w-full z-50 justify-center ">
      ...
      {headerMessage?.show && <div class="min-h-16 w-full bg-primary text-primary-content px-11 py-2 flex items-center justify-center gap-1" style={{background: headerMessage?.backgroundColor, color: headerMessage?.textColor}}>
        <p class="text-xs lg:text-base text-center font-semibold leading-[120%] flex items-center justify-center">
          <div dangerouslySetInnerHTML={{__html: headerMessage.text || ""}}/>
          {headerMessage.cta?.map((button) => (<a
                href={button?.href ?? "#"}
                target={button?.href.includes("http") ? "_blank" : "_self"}
                class={`ml-1 ${button.ctaStyle != "link" && 'btn btn-primary px-7 ml-4'} inline-flex  items-center gap-1 border-primary font-bold hover:scale-110 transition-transform text-xs lg:text-base`}
