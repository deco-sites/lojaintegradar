...
export default function Header2({ logo = {
    ...
    return (<nav id="headerContainer" class="drawer drawer-end fixed top-0 left-0 z-50 bg-primary-content bg-opacity-0 transition-all duration-300 ease-in-out pt-9 pb-5 ">
      ...
      <div class="drawer-content mx-auto w-full lg:px-0 px-4 py-0 flex gap-8 items-center justify-between max-w-[1260px] ">
        ...
        <div class="items-center justify-between w-full">
          ...
          <ul class="flex md:hidden justify-end gap-7">
            {navigation.mobileButtons?.map((item) => (<div class="flex items-center">
                <a key={item?.id} id={item?.id} href={item?.href ?? "#"} target={item?.href.includes("http") ? "_blank" : "_self"} class={` ${item.outline ? "" : ""} font-bold text-primary px-4 py-1.5 rounded-md transition-all hover:scale-110 text-xs bg-primary-content bg-opacity-60`}>
                  {item?.text}
                </a>
              </div>))}
          ...
          <ul class="hidden md:flex justify-end gap-7">
            {navigation.buttons?.map((item) => (<a key={item?.id} id={item?.id} href={item?.href ?? "#"} target={item?.href.includes("http") ? "_blank" : "_self"} class={`btn btn-primary ${item.outline ? "btn-outline" : ""} font-bold px-7 hover:scale-110 text-lg`}>
                {item?.text}
              </a>))}
            <label htmlFor="mobile-drawer-nav" class="flex btn btn-ghost drawer-button px-0">
      ...
      <aside class="drawer-side z-50 overflow-hidden">
        ...
        <div class="flex flex-col gap-8 min-h-full min-w-64 h-10 bg-base-100 text-base-content">
          <ul class="pt-8 pb-6 pl-4 pr-16 flex items-center flex-wrap gap-3 bg-accent relative">
            ...
            {navigation.asideMenuButtons?.map((item) => (<a key={item?.id} id={item?.id} href={item?.href ?? "#"} target={item?.href.includes("http") ? "_blank" : "_self"} class={`btn btn-primary ${item.outline ? "btn-outline" : ""} font-bold px-7 hover:scale-110 text-lg`}>
                {item?.text}
              </a>))}
          </ul>
          ...
          <ul class="menu carousel px-8">
            <div class="flex flex-col">
              ...
              {navigation?.links.map((link) => (<li class="border-b border-neutral last:border-none">
                  <a href={link.url} aria-label={link.label} class="text-primary hover:text-accent hover:bg-transparent text-base font-semibold flex justify-between py-6 rounded-none">
                    {link.label}
